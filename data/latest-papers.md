...
Updated: 2026-07-20T10:38:26.242934+00:00
Window: last 14 days

## 机器学习分子动力学模拟与热力学性质

### 1. Self-organized defect clustering and concentration-dependent vacancy diffusion in MoS$_2$
...
### 5. dpti: An Automated Thermodynamic Integration Workflow for Phase Diagram Calculations with Machine Learning Interatomic Potentials
...
### 6. Dyna-Mat: End-to-end benchmarking of foundation machine learning interatomic potentials in finite-temperature ensembles
...
### 8. Enerzyme: A Framework for Efficient Training of Reactive Neural Network Potentials for Enzyme Catalysis with Application to Methyltransferases
...
### 9. Can DFT-trained neural network potentials reproduce structure, solvation, and water-exchange properties in aqueous magnesium solutions?

- Source: arXiv
- Date: 2026-06-18T11:26:45Z
- Venue: physics.chem-ph, physics.comp-ph
- Authors: Sebastian Falkner, Pablo Montero de Hijes, Christoph Dellago, Nadine Schwierz
- Link: https://arxiv.org/abs/2606.20105v2
- Score: 8.0
- Match: 标题匹配 neural network potential

**中文摘要：** 镁离子在许多生物过程中发挥着重要作用，但在生物分子模拟中建模仍然具有挑战性。尽管付出了巨大的科学努力，经典力场仍无法同时再现关键的结构、热力学和动力学解特性，这可能是由于它们无法明确解释量子多体效应。 在这里，我们开发并系统地基准测试了基于 revPBE-D3/zd 和 revPBE0-D3/zd 密度泛函理论参考数据训练的 MgCl$_2$ 水溶液的 MACE 神经网络电位 (NNP)，并评估它们再现广泛实验溶液特性的能力，包括第一水合壳的结构、扩散系数、活度导数、水交换率和机制以及溶剂化自由能。两种 NNP 都准确地再现了第一水合壳层的八面体结构、离子配对特性和扩散系数。 将 NNP 与过渡路径采样和其他增强采样技术相结合，使我们能够捕获 Mg$^{2+}$ 第一个水合壳中罕见的水交换事件，揭示解离交换机制。过渡界面采样产生的交换率在实验的一个数量级内，代表着对经典解离力场的重大改进。 相比之下，NNP 衍生的溶剂化自由能显着低估了实验值，揭示了当前局部 NNP 结构在描述离子溶剂化热力学方面的局限性。我们的结果表明，经过 DFT 训练的 NNP 可以准确描述 Mg$^{2+}$ 水合结构、扩散、离子配对和交换动力学，同时强调需要明确的远程静电处理以实现与实验离子溶剂化自由能的定量一致性。
**讨论重点：** 在这里，我们开发并系统地基准测试了基于 revPBE-D3/zd 和 revPBE0-D3/zd 密度泛函理论参考数据训练的 MgCl$_2$ 水溶液的 MACE 神经网络电位 (NNP)，并评估它们再现广泛实验溶液特性的能力，包括第一水合壳的结构、扩散系数、活度导数、水交换率和机制以及溶剂化自由能。镁离子在许多生物过程中发挥着重要作用，但在生物分子模拟中建模仍然具有挑战性。 结合关键词看，阅读时应重点关注神经网络势相关的基准数据集、评价指标、误差来源，以及不同模型在精度、效率和可迁移性之间的取舍。

### 10. Distilling latent electrostatics from foundation machine learning interatomic potentials

- Source: arXiv
- Date: 2026-06-12T22:45:20Z
- Venue: physics.comp-ph, cond-mat.mtrl-sci, cs.LG
- Authors: Xiaoyu Wang, Bingqing Cheng
- Link: https://arxiv.org/abs/2606.15001v1
- Score: 8.0
- Match: 标题匹配 machine learning interatomic potential

**中文摘要：** 基础机器学习原子间势（MLIP）已经能够在化学和材料空间的广泛区域进行原子模拟，但许多模型的计算成本仍然很高，并且缺乏明确的静电学，限制了它们在由长程相互作用和电响应控制的系统中的使用。之前，我们介绍了潜在埃瓦尔德求和 (LES)，它仅从密度泛函理论 (DFT) 能量和力标签中学习潜在原子电荷和长程静电。 在这里，我们使用 LES 来提取基础模型中潜在的静电：教师模型预测的能量和力用于训练轻量级 LES 增强的学生 MLIP，并可选择对附加 DFT 数据进行微调。由此产生的模型降低了计算成本，同时提供了玻恩有效电荷张量和红外光谱。 我们对从广泛的基础 MLIP 中提取的学生模型（包括 UMA、MACE、Orb、eSEN、GemNet-OC、PET 和基于 EquiformerV2 的模型）与液态水、浓盐酸和锐钛矿 TiO2(101)-水界面的实验红外光谱进行了基准测试。在这些系统中，可以从大多数基础 MLIP 中提取静电响应。该基准进一步表明，用于训练教师模型的底层 DFT 级别和数据集在确定静电和光谱精度方面比架构发挥着更大的作用。 对于 TiO2-水界面，使用适量的高级 DFT 数据进行微调可以改善结构和红外预测。因此，基于 LES 的蒸馏提供了一种将基础 MLIP 转换为高效、电响应模型的实用途径，同时还测试基础模型中编码的物理保真度。
**讨论重点：** 我们对从广泛的基础 MLIP 中提取的学生模型（包括 UMA、MACE、Orb、eSEN、GemNet-OC、PET 和基于 EquiformerV2 的模型）与液态水、浓盐酸和锐钛矿 TiO2(101)-水界面的实验红外光谱进行了基准测试。在这里，我们使用 LES 来提取基础模型中潜在的静电：教师模型预测的能量和力用于训练轻量级 LES 增强的学生 MLIP，并可选择对附加 DFT 数据进行微调。 结合关键词看，阅读时应重点关注机器学习原子间势相关的基准数据集、评价指标、误差来源，以及不同模型在精度、效率和可迁移性之间的取舍。

## 凝聚态物理强关联体系和多铁性质

### 1. Fractional quantum ferroelectric control of spin-valley locking and valley Hall effects in altermagnetic monolayer Cr2S2

- Source: arXiv
- Date: 2026-07-16T01:23:59Z
- Venue: cond-mat.mtrl-sci
- Authors: Tao Yao, Quan Shen, Jiansheng Dong, Jianing Tan
- Link: https://arxiv.org/abs/2607.14465v1
- Score: 30.0
- Match: 摘要匹配 multiferroic; 摘要匹配 ferroelectricity; 摘要匹配 altermagnetism; 标题匹配 altermagnetic

**中文摘要：** 分数量子多铁性材料由分数量子铁电性（FQFE）和交变磁性（AM）之间的耦合产生，为零净磁化系统中动量相关自旋分裂的非易失性控制提供了一个有前景的平台。然而，将这种 FQFE-AM 耦合扩展到谷自由度和贝里曲率驱动的谷霍尔效应在很大程度上仍未得到探索。 在这里，使用第一原理计算，我们证明单层 Cr2S2 实现了一个二维 FQFE-AM 平台，该平台具有两个可切换的 FQFE 状态，通过复合对称操作将分数晶格平移与时间反转或奇偶时间反转相结合。我们证明，FQFE 切换反转了 AM 自旋极化能带结构，并互换了 X 和 Y 谷的自旋特征，而不旋转 Néel 矢量，从而实现了偏振可切换自旋谷锁定。 此外，两个 FQFE 态表现出相反的贝里曲率分布，与切换的自旋谷锁定一起，在电子和空穴掺杂下实现偏振控制的谷霍尔效应。这些结果展示了一种基于对称性的 AM 自旋分裂、自旋谷锁定和谷霍尔效应的非易失性电控制机制，为基于 FQFE-AM 耦合的低功耗谷电子器件提供了通用途径。
**讨论重点：** 分数量子多铁性材料由分数量子铁电性（FQFE）和交变磁性（AM）之间的耦合产生，为零净磁化系统中动量相关自旋分裂的非易失性控制提供了一个有前景的平台。然而，将这种 FQFE-AM 耦合扩展到谷自由度和贝里曲率驱动的谷霍尔效应在很大程度上仍未得到探索。 结合关键词看，阅读时应重点关注多铁性、铁电性、交变磁性相关的交变磁性的对称性来源、动量空间自旋劈裂，以及这些特征如何影响输运、光学或磁动力学响应。
...
- Score: 29.0
- Match: 标题匹配 multiferroic; 标题匹配 altermagnetic; 标题匹配 altermagnet; 最近 3 天发布
...
### 3. Interlayer sliding direction as a symmetry selector in altermagnetic bilayer Fe2WS4: Switchable anomalous Hall and anomalous valley Hall effects

- Source: arXiv
- Date: 2026-07-16T01:12:20Z
- Venue: cond-mat.mtrl-sci, cond-mat.mes-hall
- Authors: Quan Shen, Jianing Tan, Tao Yao, Wenhu Liao, Jiansheng Dong
- Link: https://arxiv.org/abs/2607.14459v1
- Score: 27.0
- Match: 摘要匹配 magnetoelectric coupling; 摘要匹配 ferroelectricity; 标题匹配 altermagnetic; 标题匹配 altermagnet

**中文摘要：** 交变磁体将补偿共线磁序与动量相关的自旋分裂相结合，为在没有净磁化的情况下将自旋和谷自由度与铁电和贝里曲率驱动的输运耦合提供了一个有前景的平台。然而，实现对这些相互交织的自由度的非易失性和选择性控制仍然是一个关键挑战。在这里，利用第一性原理计算，我们表明层间滑动方向可作为互磁双层 Fe2WS4 中的对称选择性控制参数。 对角滑动打破了反演对称性并产生两种具有相反面外极化的滑动铁电态。铁电极化的反转会切换动量相关的自旋纹理并反转反常霍尔电导率，揭示强磁电耦合并实现铁电可切换的反常霍尔效应。相比之下，轴向滑动保留了反演对称性，但打破了与 X 和 Y 谷相关的晶体对称性，导致可逆谷极化和可切换的反常谷霍尔效应。 这些结果将层间滑动方向确立为非易失性对称选择器，用于控制二维互磁双层中的铁电性、自旋织构、谷极化和霍尔传输响应。
**讨论重点：** 交变磁体将补偿共线磁序与动量相关的自旋分裂相结合，为在没有净磁化的情况下将自旋和谷自由度与铁电和贝里曲率驱动的输运耦合提供了一个有前景的平台。然而，实现对这些相互交织的自由度的非易失性和选择性控制仍然是一个关键挑战。 结合关键词看，阅读时应重点关注磁电耦合、铁电性、交变磁性相关的交变磁性的对称性来源、动量空间自旋劈裂，以及这些特征如何影响输运、光学或磁动力学响应。
...
### 5. Altermagnetic spin textures coupled to superconductors: Domain wall spin-triplet superconductivity and supercurrent-induced torques
...
### 6. Emergent d-wave altermagnetism in chlorine-adsorbed FeSe monolayer
...
### 7. Multistate ferroelectricity and switchable layer-locked anomalous valley Hall effects in bilayer ReIrGe2Se6

- Source: arXiv
- Date: 2026-07-16T01:17:45Z
- Venue: cond-mat.mtrl-sci
- Authors: Tao Yao, Quan Shen, Jianing Tan, Jiansheng Dong
- Link: https://arxiv.org/abs/2607.14461v1
- Score: 23.0
- Match: 摘要匹配 multiferroic; 摘要匹配 multiferroic materials; 摘要匹配 magnetoelectric coupling; 标题匹配 ferroelectricity

**中文摘要：** 二维多铁性材料将磁性和铁电 (FE) 级与强磁电耦合相结合，代表了高密度信息存储和低功耗多态电子学的理想平台。然而，传统铁电性的固有双稳定性对在单一材料内实现多个非易失性状态和可编程贝里曲率驱动的输运响应提出了重大挑战。在这里，我们利用第一性原理计算，预测了 AA0 堆叠双层 ReIrGe2Se6 中的多态铁电性。 该系统具有四种能量稳定的有限元偏振配置，其中三种通过可逆转换路径连接，而第四种则呈现单向转换路径。独特的有限元配置进一步导致电子结构中半导体-金属-半导体的循环演化。值得注意的是，FE 偏振切换与层自由度和贝里曲率密切相关。 与不同 FE 配置相关的层相关静电势控制着带边态的层特性，从而将贝里曲率锁定到特定的层通道。因此，双层 ReIrGe2Se6 能够在反常谷霍尔效应和层锁定反常谷霍尔效应之间切换，从而提供层、谷和自旋解析传输响应的非易失性控制。此外，磁化反转会切换谷和自旋通道，同时保留层分辨特征。 这些结果将双层 ReIrGe2Se6 确立为用于可编程 Berry 曲率相关传输的多态铁电平台，为基于拓扑的多功能电子和谷电子器件提供了微观洞察。
**讨论重点：** 二维多铁性材料将磁性和铁电 (FE) 级与强磁电耦合相结合，代表了高密度信息存储和低功耗多态电子学的理想平台。然而，传统铁电性的固有双稳定性对在单一材料内实现多个非易失性状态和可编程贝里曲率驱动的输运响应提出了重大挑战。 结合关键词看，阅读时应重点关注多铁性、多铁材料、磁电耦合、铁电性相关的极化翻转、磁电耦合、自旋-晶格耦合以及多铁序参量之间的相互制约。
...
### 9. Spinless charged excitation at the interface between a conventional topological insulator and a topological Mott insulator

- Source: arXiv
- Date: 2026-07-15T18:00:27Z
- Venue: cond-mat.str-el
- Authors: Cesar A. Gallegos, Andrew J. Millis, Steven R. White
- Link: https://arxiv.org/abs/2607.14221v1
- Score: 17.0
- Match: 摘要匹配 Hubbard model; 标题匹配 Mott insulator; 近两周发布

**中文摘要：** 我们使用广泛的密度矩阵重正化群计算来研究分隔两个拓扑不同的物质绝缘相的界面，以研究具有空间变化相互作用强度的三角晶格Hofstadter-Hubbard模型，选择在不同空间区域实现整数量子霍尔和手性自旋液态。我们发现整数量子霍尔-手性自旋液体界面具有与界面结合的无自旋带电激发。 界面处的这种模式是通过电荷和自旋泵浦以及通过直接计算低位激发态来识别的。我们还表征了两个相中的体激发，寻找手性自旋液体中的分级和整数量子霍尔相中自旋三重态激子形成的证据。
**讨论重点：** 我们使用广泛的密度矩阵重正化群计算来研究分隔两个拓扑不同的物质绝缘相的界面，以研究具有空间变化相互作用强度的三角晶格Hofstadter-Hubbard模型，选择在不同空间区域实现整数量子霍尔和手性自旋液态。我们发现整数量子霍尔-手性自旋液体界面具有与界面结合的无自旋带电激发。 结合关键词看，阅读时应重点关注Hubbard 模型、Mott 绝缘体相关的有效模型参数、电子关联强度、相图边界以及可与实验对照的谱学或输运信号。

### 10. Ultrafast altermagnetophononics
